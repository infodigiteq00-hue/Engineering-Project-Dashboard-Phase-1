...
const SuperAdminDashboard = () => {
  ...
  const handleCreateCompany = async () => {
    try {
      ...
      // Create company in firms table first
      ...
      if (newCompanyLogo) {
        try {
          ...
          toast({ 
            title: 'Warning', 
            description: 'Company created but logo upload failed. You can add logo later by editing the company.',
            variant: 'default'
          });
        }
      }
    ...
    try {
      ...
      if (editingCompanyLogo) {
        try {
          ...
          toast({ 
            title: 'Warning', 
            description: 'Company updated but logo upload failed. Please try again.',
            variant: 'default'
          });
        }
      }
                ...
                <div className="space-y-3">
                  {newCompanyLogoPreview ? (
                    <div className="relative">
                      <div className="bg-white border border-gray-200 rounded-lg p-3 flex items-center justify-center min-h-[120px]">
                        ...
                        onChange={(e) => {
                          ...
                          if (file) {
                            ...
                            if (file.size > 5 * 1024 * 1024) {
                              toast({
                                title: 'Error',
                                description: 'File size too large. Please select a file smaller than 5MB.',
                                variant: 'destructive'
                              });
                              return;
                            }
              ...
              <div>
                ...
                <div className="space-y-3">
                  {(editingCompanyLogoPreview || editingCompany?.logo_url) ? (
                    <div className="relative">
                      <div className="bg-white border border-gray-200 rounded-lg p-3 flex items-center justify-center min-h-[120px]">
                        {((editingCompanyLogoPreview || editingCompany?.logo_url)?.toLowerCase().endsWith('.pdf')) ? (
                          <div className="flex flex-col items-center gap-2">
                            <svg className="w-12 h-12 text-red-600" fill="currentColor" viewBox="0 0 20 20">
                              <path fillRule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z" clipRule="evenodd" />
                            </svg>
                          ...
                          <img 
                            src={editingCompanyLogoPreview || editingCompany?.logo_url || ''} 
                            alt="Logo preview" 
                            className="max-w-full max-h-[100px] object-contain"
                          />
                        )}
